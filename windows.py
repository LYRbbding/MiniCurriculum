...
class Ui_MainWindow(QtWidgets.QMainWindow):
    def setupUi(self, MainWindow):
        ...
        self.label_Week.setAlignment(QtCore.Qt.AlignRight
        ...
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(
            self.label_Week2.sizePolicy().hasHeightForWidth())
        ...
        self.label_Date.setAlignment(QtCore.Qt.AlignRight
                                     | QtCore.Qt.AlignTrailing
                                     | QtCore.Qt.AlignVCenter)
        ...
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(
            self.label_Date2.sizePolicy().hasHeightForWidth())
        ...
        self.label_Section.setAlignment(QtCore.Qt.AlignRight
                                        | QtCore.Qt.AlignTrailing
                                        | QtCore.Qt.AlignVCenter)
        ...
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(
            self.label_Section2.sizePolicy().hasHeightForWidth())
        ...
        self.label_Classroom.setAlignment(QtCore.Qt.AlignRight
                                          | QtCore.Qt.AlignTrailing
                                          | QtCore.Qt.AlignVCenter)
        ...
        self.label_Course.setAlignment(QtCore.Qt.AlignRight
                                       | QtCore.Qt.AlignTrailing
                                       | QtCore.Qt.AlignVCenter)
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(
            _translate("MainWindow", "2019211112班 2019-2020学年度第二学期课程表"))
        ...
        self.menu_Program.triggered[QtWidgets.QAction].connect(
            self.processtrigger)
        self.menu_Settings.setTitle(_translate("MainWindow", "设置"))
        self.menu_Settings.triggered[QtWidgets.QAction].connect(
            self.processtrigger)
        self.menu_Help.setTitle(_translate("MainWindow", "帮助"))
        self.menu_Help.triggered[QtWidgets.QAction].connect(
            self.processtrigger)
    ...
    def processtrigger(self, action):
        ...
        if action.text() == "导入课程表":
                    ...
                    try:
                        ...
                        for i in range(len(section_items_list)):
                            for j in range(len(date_items_list)):
                                item = self.tableWidget.item(i, j)
                                item.setText(
                                    _translate("MainWindow", classTable[i][j]))
            ...
            try:
                openfile_name = QtWidgets.QFileDialog.getOpenFileName(
                    self, '选择背景图片', '',
                    '所有图片文件(*.bmp;*.jpg;*.jpeg;*.jpe;*.png);;位图文件(*.bmp);;JPEG (*.jpg;*.jpeg;*.jpe);;PNG (*.png)'
                )
                if openfile_name[0] != '':
                    try:
                        setting = []
                        ...
                        with open('settings.ini', 'w', encoding='UTF-8') as f:
                            for i in range(len(setting)):
                                f.write(setting[i])
                        new_qss = "#MainWindow{border-image:url(" + openfile_name[
                            0] + ");}"
    ...
    def show_spirit(self):
        ...
        try:
            ...
            for i in range(0, 7):
                time_status = i
                if time_group[time_status] <= timestamp.tm_hour < time_group[
                        time_status + 1]:
                    break
            else:
                time_status = 7
            self.WidgetWindow = spirit.ShapeWidget(time_status, self,
                                                   self.MainWindow)
    ...
    def combo_Week_active(self, index):
        ...
        if self.WeekStart != week_items_list.index(
                self.comboBox_WeekStart.currentText()):
            #开始周次
            self.WeekStart = week_items_list.index(
                self.comboBox_WeekStart.currentText())
            ...
            for i in range(20 - self.WeekStart):
                self.comboBox_WeekEnd.addItem(week_items_list[self.WeekStart +
                                                              i])
        ...
        elif self.WeekEnd != week_items_list.index(
                self.comboBox_WeekEnd.currentText()):
            #结束周次
            self.WeekEnd = week_items_list.index(
                self.comboBox_WeekEnd.currentText())
    ...
    def combo_Date_active(self, index):
        ...
        if self.DateStart != date_items_list.index(
                self.comboBox_DateStart.currentText()):
            #开始星期
            self.DateStart = date_items_list.index(
                self.comboBox_DateStart.currentText())
            EndCount = self.comboBox_DateEnd.count()
            for i in range(EndCount):
                self.comboBox_DateEnd.removeItem(0)
            for i in range(5 - self.DateStart):
                self.comboBox_DateEnd.addItem(date_items_list[self.DateStart +
                                                              i])
        ...
        elif self.DateEnd != date_items_list.index(
                self.comboBox_DateEnd.currentText()):
            #结束星期
            self.DateEnd = date_items_list.index(
                self.comboBox_DateEnd.currentText())
    ...
    def combo_Section_active(self, index):
        ...
        if self.SectionStart != section_items_list.index(
                self.comboBox_SectionStart.currentText()):
            #开始节次
            self.SectionStart = section_items_list.index(
                self.comboBox_SectionStart.currentText())
            EndCount = self.comboBox_SectionEnd.count()
            for i in range(EndCount):
                self.comboBox_SectionEnd.removeItem(0)
            for i in range(14 - self.SectionStart):
                self.comboBox_SectionEnd.addItem(
                    section_items_list[self.SectionStart + i])
            if (self.SectionStart < self.SectionEnd):
                self.comboBox_SectionEnd.setCurrentIndex(self.SectionEnd -
                                                         self.SectionStart)
            else:
                self.SectionEnd = self.SectionStart
        elif self.SectionEnd != section_items_list.index(
                self.comboBox_SectionEnd.currentText()):
            #结束节次
            self.SectionEnd = section_items_list.index(
                self.comboBox_SectionEnd.currentText())
    ...
    def goBack(self):
        ...
        for i in range(len(section_items_list)):
            for j in range(len(date_items_list)):
                item = self.tableWidget.item(i, j)
                item.setText(_translate("MainWindow", classTable[i][j]))
                self.tableWidget.item(i, j).setForeground(
    ...
    def query(self):
        ...
        def get_query_str(w_s, w_e, d_s, d_e, s_s, s_e, classroom, course):
            ...
            section = ") and (section like '%" + str(
                section_items_list[s_s]) + "%'"
            for i in range(w_s + 1, w_e + 1):
                week = week + " or week_s like '%" + week_select_list[i] + "%'"
            for i in range(d_s + 1, d_e + 1):
                date = date + " or date like '%" + str(
                    date_items_list[i]) + "%'"
            for i in range(s_s + 1, s_e + 1):
                section = section + " or section like '%" + str(
                    section_items_list[i]) + "%'"
        ...
        if self.checkBox_ShowOwn.isChecked():
            ...
            for i in range(len(section_items_list)):
                for j in range(len(date_items_list)):
                    ...
                    if checkWeek(
                            i, j
                    ) and self.DateStart <= j <= self.DateEnd and self.SectionStart <= i <= self.SectionEnd and self.comboBox_Classroom.currentText(
                    ) in classTable[i][j] and self.comboBox_Course.currentText(
                    ) in classTable[i][j]:
                        ...
                        self.tableWidget.item(i, j).setForeground(
                            QtGui.QBrush(QtGui.QColor(0, 0, 0)))
                    if (int((i * 5 + j + 1) / 70 * 100) > int(
                        (i * 5 + j) / 70 * 100)):
                        self.progressBar.setProperty(
                            "value", int((i * 5 + j + 1) / 70 * 100))
            self.tableWidget.setVisible(True)
            try:
                self.tableWidget_result.setVisible(False)
            except:
            ...
            c = conn.cursor()
            query_full_str = "select * from main where " + get_query_str(
                self.WeekStart, self.WeekEnd, self.DateStart, self.DateEnd,
                self.SectionStart, self.SectionEnd,
                self.comboBox_Classroom.currentText(),
                self.comboBox_Course.currentText())
            ...
            result_title = [
                '校区', '教学楼', '教室', '周次', '日期', '节次', '课程名称', '授课教师', '班级'
            ]
            result_width = [65, 100, 100, 100, 40, 60, 220, 80, 180]
            self.tableWidget_result = QtWidgets.QTableWidget(
                self.centralwidget)
            sizePolicy = QtWidgets.QSizePolicy(
                QtWidgets.QSizePolicy.MinimumExpanding,
                QtWidgets.QSizePolicy.MinimumExpanding)
            ...
            sizePolicy.setVerticalStretch(0)
            sizePolicy.setHeightForWidth(
                self.tableWidget_result.sizePolicy().hasHeightForWidth())
            ...
            self.tableWidget_result.verticalHeader(
            ).setCascadingSectionResizes(False)
            self.tableWidget_result.verticalHeader().setSectionResizeMode(
                QtWidgets.QHeaderView.ResizeToContents)
            conn.close()
