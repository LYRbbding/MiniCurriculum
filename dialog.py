...
class Ui_Dialog(QtWidgets.QWidget):
    def setupUi(self, Dialog, status):
        ...
        if status == 'info':
            ...
            space_width = 10
        spacerItem = QtWidgets.QSpacerItem(space_width, 40, QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)
        ...
        if status == 'success':
            ...
            self.label_icon.setStyleSheet("#label_icon{border-image:url(./icons/success.png);}")
        elif status == 'warning':
            self.label.setText("已导入课程，更新数据文件失败！\n请检查数据库文件读写权限！")
            self.label_icon.setStyleSheet("#label_icon{border-image:url(./icons/warning.png);}")
        elif status == 'info':
            self.label.setText("迷你课程表开发者：\n刘怿睿\t2019210367\n杨　亮\t2019210362\n黄诗扬\t2019210374\n郭佩圣\t2019210375\n李硕\t2019210376")
            self.label_icon.setStyleSheet("#label_icon{border-image:url(./icons/warning.png);}")
            font = QtGui.QFont()
            font.setFamily("微软雅黑 Light")
            ...
            self.label_icon.setStyleSheet("#label_icon{border-image:url(./icons/fail.png);}")
